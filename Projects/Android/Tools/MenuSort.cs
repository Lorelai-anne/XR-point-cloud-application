...
public enum SceneType // types of scenes, can add if want more
{
    Programs,
    MAX
}
...
public static class MenuSort
{
    ...
    static IClass activeScene;
    static IClass nextScene;
    ...
    private static Type ActiveScene { set { nextScene = (IClass)Activator.CreateInstance(value); } }
    ...
    public static void FindScenes() //Searches through Project and finds classes implementing specific interface (i.e. IClass in this case)
    {
        allScenes = Assembly.GetExecutingAssembly()
            .GetTypes()
            .Where(a => a != typeof(IClass) && typeof(IClass).IsAssignableFrom(a))
            .OrderBy(a => a.Name)
            .ToArray();

        sceneTypes = new SceneItem[(int)SceneType.MAX][];
        sceneTypes[(int)SceneType.Programs] = allScenes
            .Where(t => t.Name.StartsWith("Show")) //Searches for classes, implementing the interface IClass, starting with 'Show'
            .Select(t => new SceneItem { type = t, name = t.Name.Substring("Show".Length) })
            .ToArray();
    }
    ...
    public static void Initialize()
    {
        ...
        if(nextScene == null)
        {
            nextScene = (IClass)Activator.CreateInstance(allScenes[sceneIndex]);
        }
    }
    ...
    public static void Update()
    {
        ...
        if(IsTesting && FinishedWithClass())
        {
            sceneIndex += 1;
            if (sceneIndex >= allScenes.Length || TestSingle)
                SK.Quit();
            else
                SetClassActive(allScenes[sceneIndex].Name);
        }
    }
    ...
    public static string GetClassName(SceneType category, int index)
    {
        return sceneTypes[(int)category][index].name;
    }
    public static void SetClassActive(SceneType category, int index)
    {
        Log.Info($"Starting Scene: {sceneTypes[(int)category][index].name}");
        ActiveScene = sceneTypes[(int)category][index].type;
    }
    public static void SetClassActive(string name)
    {
        name = name.ToLower();
    ...
    public static void Scene(Func<bool> testFunction)
    {
        try
        {
            if (!testFunction())
    ...
    private static bool FinishedWithClass()
    {
        if(runSeconds != 0) return Time.Totalf-sceneTime > runSeconds;
        if (runFrames != -1) return sceneTime == runFrames;
        return true;
    }
